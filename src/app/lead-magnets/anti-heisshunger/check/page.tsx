      ...
      <section className="mx-auto max-w-5xl space-y-8">
        ...
        <section className="grid gap-5 lg:grid-cols-[1fr_0.85fr]">
          <div className="rounded-3xl border border-slate-800 bg-slate-900 p-6">
            ...
            <div className="mt-5 space-y-6">
              ...
              <div>
                ...
                <textarea
                  className="mt-3 min-h-24 w-full rounded-2xl border border-slate-700 bg-slate-950 p-4 text-slate-100 outline-none focus:border-rose-300"
                  placeholder="z. B. Schokolade, Kekse, Eis..."
                  value={foods}
              ...
              <div>
                <label className="text-sm font-black uppercase tracking-[0.18em] text-rose-200">Optional: E-Mail für Auswertung</label>
                <input
                  className="mt-3 w-full rounded-2xl border border-slate-700 bg-slate-950 p-4 text-slate-100 outline-none focus:border-rose-300"
          ...
          <aside className="rounded-3xl border border-emerald-300/30 bg-emerald-300/10 p-6 text-emerald-50">
            ...
            <p className="mt-4 leading-8 text-emerald-50/90">
              {complete ? result.text : "Sobald alle Pflichtfelder ausgefüllt sind, erscheint hier eine klare Einschätzung mit nächster Handlung."}
            </p>
            {complete && (
              <div className="mt-6 rounded-2xl bg-slate-950/70 p-5">
                ...
                {email && <p className="mt-3 text-sm text-slate-300">Kontakt: {email}</p>}
              </div>
            )}
            <div className="mt-6 grid gap-3">
              <button
                ...
                className="rounded-full bg-emerald-200 px-6 py-3 text-sm font-black text-emerald-950 hover:bg-white disabled:cursor-not-allowed disabled:opacity-50"
                disabled={!complete}
              >
                Auswertung kopieren
              </button>
              <Link href="/lead-magnets/anti-heisshunger" className="rounded-full border border-slate-500 px-6 py-3 text-center text-sm font-bold text-slate-200 hover:bg-white hover:text-slate-950">
                Zurück zum E-Book
              </Link>
            </div>
          </aside>
        ...
        {options.map((option) => {
          ...
          return (
            <button
              key={option}
              type="button"
              onClick={() => setData((current) => ({ ...current, [name]: option }))}
              className={`rounded-full border px-4 py-2 text-sm font-bold transition ${
                active ? "border-rose-200 bg-rose-200 text-rose-950" : "border-slate-700 bg-slate-950 text-slate-200 hover:border-rose-300"
              }`}
            >
              {option}
            </button>
