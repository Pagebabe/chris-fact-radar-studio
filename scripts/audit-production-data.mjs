...
function fail(check, detail, meta) {
  add("FAIL", check, detail, meta);
}

function warn(check, detail, meta) {
  add("WARN", check, detail, meta);
}

function pass(check, detail, meta) {
  add("PASS", check, detail, meta);
}
...
function checkUrl(value, context, { allowExample = false } = {}) {
  ...
  for (const pattern of FORBIDDEN_URL_PATTERNS) {
    if (allowExample && pattern.source.includes("example\\.com")) continue;
...
function auditHtml(htmlByEndpoint) {
  ...
  const forbiddenCopy = [
    ...
    ["20 reviewed cases across 13 sources", /20 reviewed cases across 13 sources/i],
    ["Video-Walkthrough erscheint in Kürze", /erscheint (?:hier )?in Kürze/i],
    ["absoluter Plattform-Claim", /der einzige Engpass|kein weiterer Umbau nötig/i],
  ];
...
async function main() {
  ...
  await writeFile(path.join(REPORT_DIR, "production-audit.md"), markdown, "utf8");
