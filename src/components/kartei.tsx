...
export function Kartei({ creators, claims, onToggleWatch, onOpenClaim }: Props) {
  ...
  function toggleWatch(creator: CreatorRecord) {
  ...
  return (
    <div className="kartei-layout">
              ...
              <article key={c.id} className={`creator-card${selected?.id === c.id ? " selected" : ""}`}>
                <button className="creator-card-main" onClick={() => setSelectedId(c.id)} aria-pressed={selected?.id === c.id}>
                  ...
                  <span className="creator-card-copy">
                    ...
                    <span>{creatorMeta(c)}</span>
                  </span>
                  <span className={`severity-badge severity-${sev}`}>{sev}</span>
                </button>
                <div className="creator-card-stats">
                  <span><strong>{c.falschaussagenCount}</strong> Claims</span>
                  <span><strong>{(c.totalViews / 1_000_000).toFixed(1)} Mio.</strong> Views</span>
                </div>
                <button className={`watch-btn${watched ? " active" : ""}`} onClick={() => toggleWatch(c)}>
                  {watched ? "Auf Watchlist" : "Beobachten"}
                </button>
              </article>
            );
          })}
      ...
      <aside className="kartei-detail" aria-label="Akte Detail">
        {selected ? (
          <>
            <div className="kartei-detail-head">
              <p className="section-label">Akte</p>
              <h3>{selected.name}</h3>
              <p>{creatorMeta(selected)}</p>
            </div>
            <div className="detail-stat-grid">
              <span><strong>{selected.falschaussagenCount}</strong> problematische Claims</span>
              <span><strong>{(selected.totalViews / 1_000_000).toFixed(1)} Mio.</strong> Views</span>
              <span><strong>{selected.damageScore}</strong> Risiko</span>
            </div>
            <div className="creator-summary">
              <h4>Warum relevant?</h4>
              <p>{selected.note ?? "Keine Notiz vorhanden."}</p>
            </div>
            ...
            <button className="primary-btn" onClick={() => exportDossier(selected)}>
              {copied ? "Akte kopiert" : "Akte als Markdown kopieren"}
            </button>
          </>
        ) : (
